...
<body class="bg-gray-50 min-h-screen">
            ...
            <div class="flex justify-between h-16 items-center">
                <div class="flex items-center flex-wrap">
                    ...
                    <div class="ml-10 flex items-center space-x-4 flex-wrap">
                        ...
                        <a href="reports.php" class="text-white px-3 py-2 text-sm font-medium">Reports</a>
                ...
                <div class="flex items-center space-x-4 flex-wrap">
                    ...
                    <button id="exportBtn" class="bg-secondary text-primary px-4 py-2 !rounded-button flex items-center">
    ...
    <div class="max-w-7xl mx-auto px-4 py-6 pt-20"> <!-- Added pt-20 to account for fixed navbar -->
        <div class="bg-white rounded-lg shadow p-6 mb-6">
            <div class="flex flex-wrap gap-4 mb-6">
                <div class="flex-1 min-w-[300px] relative">
                    <input type="text" id="searchInput" placeholder="Search health data..." class="w-full pl-10 pr-4 py-2 border border-gray-200 rounded focus:outline-none focus:border-primary">
                    <i class="ri-search-line absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                </div>
                <div class="flex gap-4 flex-wrap">
                    <div class="relative">
                        <input type="date" id="dateFilter" class="date-input pl-10 pr-4 py-2 border border-gray-200 rounded focus:outline-none focus:border-primary">
                        <i class="ri-calendar-line absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"></i>
                    </div>
                    <div class="relative">
                        <button id="filterBtn" class="px-4 py-2 border border-gray-200 rounded flex items-center">
            ...
            <div class="flex flex-wrap gap-4 mb-6" id="categoryButtons">
                <button class="px-4 py-2 bg-primary text-white !rounded-full active" data-category="vital">Vital Statistics</button>
                <button class="px-4 py-2 bg-gray-100 text-gray-600 !rounded-full" data-category="renal">Renal Tests</button>
            </div>

            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <div class="bg-white rounded-lg shadow p-4" id="vitalChartContainer">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-lg font-semibold text-gray-900">Vital Statistics Trends</h3>
                        <button class="text-primary hover:text-primary/80">
                            <i class="ri-more-2-fill"></i>
                        </button>
                    </div>
                    <div id="vitalChart" style="width: 100%; height: 300px;"></div>
                </div>

                <div class="bg-white rounded-lg shadow p-4" id="renalChartContainer" style="display: none;">
                    <div class="flex justify-between items-center mb-4">
                        <h3 class="text-lg font-semibold text-gray-900">Renal Tests Trends</h3>
                        <button class="text-primary hover:text-primary/80">
                            <i class="ri-more-2-fill"></i>
                        </button>
                    </div>
                    <div id="renalChart" style="width: 100%; height: 300px;"></div>
                </div>
            </div>

            <div class="bg-white rounded-lg shadow overflow-hidden">
                <div class="p-4 border-b border-gray-200">
                    <h3 class="text-lg font-semibold text-gray-900">Detailed Health Data</h3>
                </div>
                    ...
                    <table class="w-full">
                        <thead class="bg-gray-50">
                            <tr>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Fasting Blood Sugar</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Post Meal Blood Sugar</th>
                                ...
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Urea</th>
                                <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Creatinine</th>
                            </tr>
                        </thead>
                        <tbody class="bg-white divide-y divide-gray-200" id="healthDataTable">
                            <?php if (!empty($previous_vitals) || !empty($previous_renals)): ?>
                                <?php foreach (array_merge($previous_vitals, $previous_renals) as $record): ?>
                                    <tr>
                                        <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($record['record_date']); ?></td>
                                        <?php if (isset($record['systolic_bp'])): ?>
                                            <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($record['fasting_blood_sugar'] ?? 'N/A'); ?> mg/dL</td>
                                            <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($record['post_meal_blood_sugar'] ?? 'N/A'); ?> mg/dL</td>
                                            <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($record['systolic_bp'] ?? 'N/A') . '/' . htmlspecialchars($record['diastolic_bp'] ?? 'N/A'); ?> mmHg</td>
                                            <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($record['hdl_cholesterol'] ?? 'N/A') . '/' . htmlspecialchars($record['ldl_cholesterol'] ?? 'N/A'); ?> mg/dL</td>
                                        ...
                                        <?php else: ?>
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap">-</td>
                                            <td class="px-6 py-4 whitespace-nowrap">-</td>
                                            <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($record['urea'] ?? 'N/A'); ?> mg/dL</td>
                                            <td class="px-6 py-4 whitespace-nowrap"><?php echo htmlspecialchars($record['creatinine'] ?? 'N/A'); ?> mg/dL</td>
                                        <?php endif; ?>
                                    </tr>
                                <?php endforeach; ?>
                            <?php else: ?>
                                <tr><td colspan="7" class="px-6 py-4 text-center text-gray-600">No previous data available.</td></tr>
                            <?php endif; ?>
                        </tbody>
                    </table>
                </div>
        ...
        <div class="bg-white rounded-lg p-6 w-full max-w-md">
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <select id="dateRange" class="w-full border border-gray-200 rounded p-2">
                ...
                <div>
                    ...
                    <div class="space-y-2">
                        <label class="flex items-center">
                            <input type="checkbox" id="vitalFilter" class="rounded text-primary" checked>
                            <span class="ml-2">Vital Statistics</span>
                        </label>
                        <label class="flex items-center">
                            <input type="checkbox" id="renalFilter" class="rounded text-primary" checked>
                            <span class="ml-2">Renal Tests</span>
                        </label>
                    </div>
                </div>
                <div class="flex justify-end space-x-3 mt-6">
                    <button onclick="toggleFilterModal()" class="px-4 py-2 border border-gray-200 rounded text-gray-600">Cancel</button>
                    <button onclick="applyFilters()" class="px-4 py-2 bg-primary text-white !rounded-button">Apply Filters</button>
                </div>
            </div>
        </div>
    </div>
